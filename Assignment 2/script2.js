...
const orangeMaterial = new THREE.MeshStandardMaterial({
    color: new THREE.Color('rgb(102, 255, 0)')
})
const pinkMaterial = new THREE.MeshStandardMaterial({
    color: new THREE.Color('rgb(255, 48, 186)')
})
const aquaMaterial = new THREE.MeshStandardMaterial({
    color: new THREE.Color('rgb(124, 207, 255)')
})
...
const animation = () =>
{
    ...
    {
        camera.position.x = Math.sin(elapsedTime * 0.4) * 16
        camera.position.z = Math.cos(elapsedTime * 0.4) * 16
    }
    ...
    if(uiobj.animateBubbles){
        for(let i=0; i < scene.children.length; i++)
        {
            ...
            {
                // Set vertical oscillation motion
                const verticalOffset = Math.sin(elapsedTime * scene.children[i].randomizer) * 5
                scene.children[i].position.y = verticalOffset
            }
        }
    }
